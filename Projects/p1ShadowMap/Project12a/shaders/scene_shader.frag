...
float shadow()
{
  ...
  float ShadowCoordZ = (ShadowCoord.z/ShadowCoord.w);
  float shadowDepth = (texture(ShadowMap, vec2(ShadowCoordX,ShadowCoordY))).x;
  if(ShadowCoordX <0.0 || ShadowCoordX >1.0 ) return 0.0;
  if(ShadowCoordY <0.0 || ShadowCoordY >1.0 ) return 0.0;
  if(ShadowCoordZ >= shadowDepth) return 0.0;  
  return 1.0;
}
